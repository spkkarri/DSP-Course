<Faculty Notes — Lecture 29: Matched Z-Transform & Spectral Transformations>
...
1. Formulate the Matched Z-Transform (MZT): Direct pole-zero mapping $(s - s_k) \to (1 - e^{s_k T_d} z^{-1})$.
...
3. Formulate the **Constantinides Digital Spectral Transformations**:
   * Lowpass to Lowpass: $z^{-1} \to \frac{z^{-1} - \alpha}{1 - \alpha z^{-1}}$
   * Lowpass to Highpass: $z^{-1} \to -\frac{z^{-1} + \alpha}{1 + \alpha z^{-1}}$
