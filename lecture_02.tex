...
\subsection{Complex Exponentials as Eigenfunctions (Frequency Response Theorem)}
A complex exponential sequence $x[n] = e^{j\omega n}$ is an \textbf{eigenfunction} of any LTI system. That is, convolving it with an impulse response $h[n]$ yields:
\begin{equation}
...
\textit{Proof:}
\begin{equation}
y[n] = \sum_{k=-\infty}^{\infty} h[k] e^{j\omega(n-k)} = \left( \sum_{k=-\infty}^{\infty} h[k] e^{-j\omega k} \right) e^{j\omega n} = H(e^{j\omega}) e^{j\omega n}
\end{equation}

\subsection{Duality: Complex Exponential System as a Frequency Extractor}
...
\textit{Proof:}
\begin{equation}
y[n] = \sum_{k=-\infty}^{\infty} x[k] e^{j\omega_0(n-k)} = \left( \sum_{k=-\infty}^{\infty} x[k] e^{-j\omega_0 k} \right) e^{j\omega_0 n} = X(e^{j\omega_0}) e^{j\omega_0 n}
\end{equation}
...
\subsection{Note on Sign Conventions \& Pole Stability}
...
\begin{enumerate}
    \item \textbf{Left-Hand Side (LHS) Form:} If we write the first-order system with all terms on the LHS:
    \begin{equation}
    y[n] + a_1 y[n-1] = x[n]
    \end{equation}
    ...
    \item \textbf{Right-Hand Side (RHS) Form (Feedback Form):} If we express the system with feedback on the RHS:
    \begin{equation}
    y[n] = r y[n-1] + x[n]
    \end{equation}
...
\subsection{Analytical Solution Concrete Walkthrough}
Solve the difference equation $y[n] - 0.5 y[n-1] = x[n]$ for $x[n] = 2^n u[n]$ under initial rest conditions ($y[-1] = 0$).
\begin{enumerate}
    \item \textbf{Homogeneous Solution $y_h[n]$:} Set input $x[n] = 0$. Substitute $y_h[n] = C \lambda^n$:
    \begin{equation}
    ...
    \item \textbf{Particular Solution $y_p[n]$:} Since input is $x[n] = 2^n u[n]$, assume $y_p[n] = K \cdot 2^n$ for $n \ge 0$. Substitute into difference equation:
    \begin{equation}
    ...
    \end{equation}
    Thus, $y_p[n] = \frac{4}{3} 2^n$ for $n \ge 0$.
    
    \item \textbf{Complete Solution $y[n]$:} Combine the components:
    \begin{equation}
    y[n] = C (0.5)^n + \frac{4}{3} 2^n \quad (n \ge 0)
    \end{equation}
    
    \item \textbf{Solve for Constant $C$:} Under initial rest $y[-1] = 0$. Direct evaluation of difference equation at $n=0$:
    \begin{equation}
    y[0] = 0.5 y[-1] + x[0] = 0.5(0) + 2^0 = 1
    \end{equation}
    Substitute $n=0$ into complete solution:
    \begin{equation}
    y[0] = C + \frac{4}{3} \Rightarrow C + \frac{4}{3} = 1 \Rightarrow C = -\frac{1}{3}
    \end{equation}
...
\section{Hardware Implementation: Parallelization \& Systolic MAC Arrays}
In real-time digital signal processing systems (FPGAs, ASICs, and dedicated DSP processors), computing convolution sequentially is often too slow. Instead, the computation is parallelized using a \textbf{Systolic MAC (Multiply-Accumulate) Array} in the \textbf{Transposed Direct Form FIR} structure. This reduces the time complexity from $O(L_x \cdot L_h)$ to $O(L_x)$ overall, yielding $O(1)$ output throughput (one output sample per clock cycle).

\subsection{Transposed Direct Form FIR Structure}
...
Broadcast Input $x[n]$ & $\rightarrow$ & $\downarrow$ & & $\downarrow$ & & $\downarrow$ \\
& & \boxed{\times h[0]} & & \boxed{\times h[1]} & & \boxed{\times h[2]} \\
& & $\downarrow$ & & $\downarrow$ & & $\downarrow$ \\
$y[n]$ & $\leftarrow$ & \boxed{+} $\leftarrow$ & \boxed{\text{Reg } R_0} $\leftarrow$ & \boxed{+} $\leftarrow$ & \boxed{\text{Reg } R_1} $\leftarrow$ & \boxed{\text{Reg } R_2}
\end{tabular}
\end{center}

\subsection{Register Update Equations}
