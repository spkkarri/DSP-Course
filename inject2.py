...
injection = '''                    <section class="card">
                        ...
                        <div class="math-card">
                            <strong>Direct Form I</strong>
                            \\\\[ y[n] = \\sum_{k=0}^{M} b_k x[n-k] - \\sum_{k=1}^{N} a_k y[n-k] \\\\]
                            Implements the numerator and denominator as separate delay lines. It requires \\( M + N \\) delays and is straightforward but memory intensive.
                        </div>

                        <div class="math-card">
                            <strong>Direct Form II (Canonical Form)</strong>
                            \\\\[ v[n] = x[n] - \\sum_{k=1}^{N} a_k v[n-k] \\\\]
                            \\\\[ y[n] = \\sum_{k=0}^{M} b_k v[n-k] \\\\]
                            By swapping the FIR and all-pole sections, we can share the delay line. This canonical form requires only \\( \\max(M, N) \\) delays, thus minimizing memory footprint.
                        </div>

                        <div class="math-card">
                            <strong>Cascade Form</strong>
                            \\\\[ H(z) = G \\prod_{k=1}^{K} H_k(z) \\\\]
                            Factors the transfer function into a product of 2nd-order sections (biquads). Highly robust against <span style="color: #8b5cf6;">coefficient sensitivity</span>, which otherwise plagues high-order Direct Form filters.
                        </div>

                        <div class="math-card">
                            <strong>Parallel Form</strong>
                            \\\\[ H(z) = C + \\sum_{k=1}^{K} H_k(z) \\\\]
                            Uses partial fraction expansion (PFE) to create a sum of 2nd-order sections. This groups complex conjugate poles into real sections and enables simultaneous processing.
                        </div>
...
pattern = r'\s*<section class="card">\s*<div class="card-title">Core Theory: IIR Filter Structures</div>.*?(?=<!-- L13 Section 2)'
...
if count > 0:
    ...
    # If not found, insert before the comment
